...
def preprocessing_twitter():
    ...
    with open("static/files/Data Scraping.csv", "r",encoding='utf-8') as csvfile:
        ...
        for row in readCSV:
            ...
            tweets =[row[0], row[1], row[2], clean, casefold, tokenizing, normalized_text, stop_wr, stemming]
...
def proses_klasifikasi():
    ...
    # netral = counter["Netral"] netral  , 'Netral'  ,'#0000FF' if isNeutral == True else 0

    sizes = [positif, negatif]
    ...
    plt.xlabel("Tweet tentang Chatgpt")
    plt.ylabel("Jumlah Tweet")
    plt.title("Presentase Sentimen Tweet Chatgpt")
