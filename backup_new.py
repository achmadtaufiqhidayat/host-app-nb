    ...
    try:
        with open(data_preprocessing_csv_path, "r", encoding='utf-8') as preprocessing_file, \
            ...
            for row in csv_reader:
                tweet = {}
                try:
                    value = translator.translate(row[2], dest='en')  # Changed index from 7 to 2
                ...
                tweets = [row[1], row[2], labeling]  # Changed index from 7 to 2
...
def klasifikasi():
    ...
    if akurasi == 0:
        ...
        return render_template('klasifikasi.html', accuracy=akurasi, titles1=df.columns.values, table2=html_table2, titles2=df2.columns.values)
...
def tfidf_results():
    ...
    data = {'Kata': terms, 'DF': df, 'IDF': 1 + df, 'TF-IDF' : ''}
    
    # for i, term in enumerate(terms):
    #     data['TF-IDF'] = tfidf_matrix[:, i]
